...
from tests.unit.contexts.campaign.application.fakes import FakeSceneRepository
...
@pytest.fixture
def access(game_master: UserId) -> SceneAccess:
...
@pytest.fixture
def other_access(game_master: UserId) -> SceneAccess:
    return CampaignAccess(game_master).narrative_at(Unsafe(Campaign(name="Fen Wardens", owner_id=game_master)))


@pytest.fixture
def service(scenes: FakeSceneRepository):
    return SceneService(scenes)


class TestCreate:
    async def test_returns_a_scene_with_the_campaign_off_the_token(self, service: SceneService, access: SceneAccess):
    ...
    async def test_a_scene_starts_unwritten_and_planned(self, service: SceneService, access: SceneAccess):
        scene = await service.create(access, "The parley at Stonegate")

        assert scene.body == ""
        assert scene.status is SceneStatus.PLANNED

    async def test_the_first_scene_leaves_room_above_it(self, service: SceneService, access: SceneAccess):
        scene = await service.create(access, "Arrival at dusk")

        assert scene.position == POSITION_GAP

    async def test_each_new_scene_goes_after_the_last(self, service: SceneService, access: SceneAccess):
    ...
    async def test_scenes_are_placed_within_their_own_campaign(
        self, service: SceneService, access: SceneAccess, other_access: SceneAccess
    ):
    ...
    async def test_a_body_is_stored_byte_for_byte(self, service: SceneService, access: SceneAccess):
...
class TestGetFor:
    async def test_the_game_master_sees_a_scene_in_their_campaign(self, service: SceneService, access: SceneAccess):
    ...
    async def test_a_scene_that_never_existed_is_not_found(self, service: SceneService, access: SceneAccess):
        with pytest.raises(SceneNotAvailable):
            await service.get_for(SceneId(uuid.uuid4()), access)


class TestListFor:
    async def test_lists_in_narrative_order(self, service: SceneService, access: SceneAccess):
    ...
    async def test_lists_nothing_for_a_campaign_with_no_scenes(self, service: SceneService, access: SceneAccess):
...
class TestUpdate:
    async def test_rewrites_the_scene(self, service: SceneService, access: SceneAccess):
    ...
    async def test_leaves_the_scene_where_it_was(self, service: SceneService, access: SceneAccess):
...
class TestDelete:
    async def test_removes_the_scene(self, service: SceneService, access: SceneAccess):
