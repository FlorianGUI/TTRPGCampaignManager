...
class TestCreate:
    async def test_returns_campaign_with_correct_fields(self, service: CampaignService, owner_id: UserId):
    ...
    async def test_description_is_optional(self, service: CampaignService, owner_id: UserId):
        campaign = await service.create("The Hollow Beneath Greyfen", owner_id)

        assert campaign.description is None

    async def test_gives_each_campaign_its_own_id(self, service: CampaignService, owner_id: UserId):
...
class TestGetFor:
    async def test_returns_campaign_when_found(self, service: CampaignService, owner_id: UserId):
        created = await service.create("The Hollow Beneath Greyfen", owner_id)

        assert await service.get_for(created.id, owner_id) == created

    async def test_raises_when_not_found(self, service: CampaignService, owner_id: UserId):
        with pytest.raises(CampaignNotReachable):
            await service.get_for(CampaignId(uuid.uuid4()), owner_id)

    async def test_raises_when_owned_by_someone_else(
        self, service: CampaignService, owner_id: UserId, someone_else: UserId
    ):
        created = await service.create("The Hollow Beneath Greyfen", owner_id)
...
class TestListFor:
    async def test_returns_empty_list_when_no_campaigns(self, service: CampaignService, owner_id: UserId):
...
class TestAccessTo:
    async def test_grants_a_token_for_my_own_campaign(self, service: CampaignService, owner_id: UserId):
        created = await service.create("The Hollow Beneath Greyfen", owner_id)
    ...
    async def test_grants_nothing_for_a_campaign_that_does_not_exist(self, service: CampaignService, owner_id: UserId):
        with pytest.raises(CampaignNotReachable):
            await service.characters_at(CampaignId(uuid.uuid4()), owner_id)
...
class TestUpdate:
    async def test_returns_the_campaign_with_its_new_name(self, service: CampaignService, owner_id: UserId):
    ...
    async def test_clears_a_description_that_is_left_out(self, service: CampaignService, owner_id: UserId):
    ...
    async def test_raises_when_not_found(self, service: CampaignService, owner_id: UserId):
        with pytest.raises(CampaignNotReachable):
            await service.update(CampaignId(uuid.uuid4()), owner_id, "Greyfen")
...
class TestDelete:
    async def test_removes_the_campaign(self, service: CampaignService, owner_id: UserId):
        created = await service.create("Greyfen", owner_id)
    ...
    async def test_takes_the_characters_at_that_table_with_it(
        self, service: CampaignService, characters: FakeCharacterRepository, owner_id: UserId
    ):
        created = await service.create("Greyfen", owner_id)
        access = await service.characters_at(created.id, owner_id)
    ...
    async def test_deleting_an_empty_campaign_is_not_an_error(self, service: CampaignService, owner_id: UserId):
        created = await service.create("Greyfen", owner_id)
    ...
    async def test_raises_when_not_found(self, service: CampaignService, owner_id: UserId):
        with pytest.raises(CampaignNotReachable):
            await service.delete(CampaignId(uuid.uuid4()), owner_id)
